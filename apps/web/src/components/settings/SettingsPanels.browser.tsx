...
const createDesktopBridgeStub = (overrides?: {
  ...
  return {
    ...
    getCaqliAuthState: vi.fn().mockResolvedValue({ connected: false, maskedKey: null }),
    setCaqliApiKey: vi.fn().mockResolvedValue({ connected: true, maskedKey: "caqli_12...cdef" }),
    clearCaqliApiKey: vi.fn().mockResolvedValue({ connected: false, maskedKey: null }),
    getCaqliCredits: vi.fn().mockResolvedValue(null),
    readCaqliWorkspaceTree: vi.fn().mockResolvedValue({ rootPath: "/tmp", entries: [] }),
    readCaqliFile: vi.fn().mockResolvedValue(""),
    sendCaqliAgentMessage: vi.fn().mockResolvedValue({
      message: "ok",
      toolEvents: [],
      credits: null,
    }),
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(
        page.getByText("Caqli Desktop will restart to expose this environment over the network."),
      )
      .toBeInTheDocument();
