...
function makeDesktopBridge(overrides: Partial<DesktopBridge> = {}): DesktopBridge {
  return {
    ...
    getCaqliAuthState: async () => ({ connected: false, maskedKey: null }),
    setCaqliApiKey: async () => ({ connected: true, maskedKey: "caqli_12...cdef" }),
    clearCaqliApiKey: async () => ({ connected: false, maskedKey: null }),
    getCaqliCredits: async () => null,
    readCaqliWorkspaceTree: async () => ({ rootPath: "/tmp", entries: [] }),
    readCaqliFile: async () => "",
    sendCaqliAgentMessage: async () => ({ message: "ok", toolEvents: [], credits: null }),
