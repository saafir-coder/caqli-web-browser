...
const APP_USER_MODEL_ID = isDevelopment ? "com.caqli.desktop.dev" : "com.caqli.desktop";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "caqli-desktop-dev.desktop" : "caqli-desktop.desktop";
const LINUX_WM_CLASS = isDevelopment ? "caqli-desktop-dev" : "caqli-desktop";
const USER_DATA_DIR_NAME = isDevelopment ? "caqli-desktop-dev" : "caqli-desktop";
const LEGACY_USER_DATA_DIR_NAMES = isDevelopment
  ? ["Caqli AI (Dev)", "T3 Code (Dev)"]
  : ["Caqli AI (Alpha)", "T3 Code (Alpha)"];
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Caqli Desktop ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Caqli Desktop (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`caqli-desktop`). If a legacy
