...
export async function GET(req: NextRequest) {
  ...
  // Get last message time and total message count per user
  const { data: allMessages } = await supabase
    .from('messages')
    .select('user_id, created_at')
    .order('created_at', { ascending: false })

  const messageStatsMap = new Map<string, { last_active: string; total_messages: number }>()
  for (const msg of allMessages ?? []) {
    const existing = messageStatsMap.get(msg.user_id)
    if (!existing) {
      messageStatsMap.set(msg.user_id, { last_active: msg.created_at, total_messages: 1 })
    } else {
  ...
  const result = (users ?? []).map(u => ({
    ...
    last_active: messageStatsMap.get(u.id)?.last_active ?? null,
    total_messages: messageStatsMap.get(u.id)?.total_messages ?? 0,
  }))
